 ...
 */
async function initializeUsers(state: AutonomousState): Promise<AutonomousState> {
  const config = state.simulationConfig;

  console.log(chalk.blue("\n" + "=".repeat(60)));
  console.log(chalk.blue("🤖 AUTONOMOUS SIMULATION - Initializing Agents"));
  console.log(chalk.blue("=".repeat(60)));
  console.log(chalk.gray(`Creating ${config.homeownerCount} homeowners, ${config.tradespersonCount} tradespersons, ${config.businessCount} businesses`));
  ...
  for (const { role, count } of usersToCreate) {
    for (let i = 0; i < count; i++) {
      try {
        ...
        console.log(chalk.green(`  ✓ Created ${role}: ${user.fullName} (${user.email})`));
  ...
  const homeowners = state.testUsers.filter(u => u.role === "homeowner").length;
  const tradespersons = state.testUsers.filter(u => u.role === "tradesperson").length;
 ...
 */
async function autonomousLoop(state: AutonomousState): Promise<AutonomousState> {
  const config = state.simulationConfig;
  ...
  for (const agent of shuffledAgents) {
    ...
    if (state.waitingAgents.has(agent.id) && waitCycles > config.maxWaitCycles) {
      continue;
    }
    ...
    // Log agent status
    const roleEmoji = agent.role === "homeowner" ? "🏠" :
    ...
    try {
      // Decide what to do
      const decision = await decideNextAction(state, agent, goals, config.useLLM);

      if (!decision) {
        console.log(chalk.gray(`  ${roleEmoji} ${agent.fullName}: waiting (no productive action)`));
        state.waitingAgents.add(agent.id);
        continue;
      }

      console.log(chalk.white(`  ${roleEmoji} ${agent.fullName} → ${chalk.yellow(decision.action)}`));
      ...
      console.log(chalk.red(`  ${roleEmoji} ${agent.fullName}: Error - ${error}`));
...
function finalize(state: AutonomousState): AutonomousState {
  ...
  const offersGoal = state.offers.filter(o => o.status === "ACCEPTED" || o.status === "COMPLETED").length >= goals.minAcceptedOffers;
  const reviewsGoal = state.reviews.length >= goals.minReviews;

  console.log(chalk.white("\nGoal Achievement:"));
  console.log(`  ${jobsGoal ? chalk.green("✓") : chalk.red("✗")} Jobs: ${state.jobs.length}/${goals.minJobs}`);
  console.log(`  ${appsGoal ? chalk.green("✓") : chalk.red("✗")} Applications: ${state.applications.length}/${goals.minApplications}`);
  console.log(`  ${offersGoal ? chalk.green("✓") : chalk.red("✗")} Accepted Offers: ${state.offers.filter(o => o.status === "ACCEPTED" || o.status === "COMPLETED").length}/${goals.minAcceptedOffers}`);
  console.log(`  ${reviewsGoal ? chalk.green("✓") : chalk.red("✗")} Reviews: ${state.reviews.length}/${goals.minReviews}`);
  ...
  for (const user of state.testUsers) {
    ...
    const roleEmoji = user.role === "homeowner" ? "🏠" :
                      user.role === "tradesperson" ? "🔧" : "🏢";
    console.log(chalk.gray(`  ${roleEmoji} ${user.fullName}: ${successes}/${userActions.length} successful actions`));
  }
...
export async function runAutonomousSimulation(
  ...
  console.log(chalk.gray(`LLM Decisions: ${fullConfig.useLLM ? "enabled" : "disabled (heuristics only)"}`));
  console.log(chalk.gray(`Goals: ${fullConfig.goals.minJobs} jobs, ${fullConfig.goals.minApplications} apps, ${fullConfig.goals.minAcceptedOffers} offers, ${fullConfig.goals.minReviews} reviews`));
