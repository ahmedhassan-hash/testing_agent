...
import type { TestAgentState, TestUser, ActionResult, UserRole } from "../state/types.js";
import { homeownerAgent, getAvailableHomeownerActions } from "./homeowner.agent.js";
import { tradespersonAgent, getAvailableTradespersonActions } from "./tradesperson.agent.js";
import { businessAgent, getAvailableBusinessActions } from "./business.agent.js";
import { chat } from "../config/llm.js";

// Types for autonomous operation
export type HomeownerAction = "create_job" | "view_applications" | "shortlist_applicant" | "make_offer" | "complete_job" | "leave_review";
export type TradespersonAction = "browse_jobs" | "apply_to_job" | "view_offers" | "accept_offer" | "reject_offer" | "view_reviews";
export type BusinessAction = "create_job" | "view_applications" | "make_offer" | "view_offers" | "complete_job" | "leave_review";
...
function getAgentContext(state: TestAgentState, user: TestUser): string {
  ...
  const myApplications = state.applications.filter(a => a.applicantId === user.id);
  const myOffers = state.offers.filter(o =>
    o.homeownerId === user.id || o.tradespersonId === user.id
  );
  ...
  const pendingOffersForMe = myOffers.filter(o =>
    o.status === "PENDING" && o.tradespersonId === user.id
  );

  // Accepted offers I can complete (homeowner/business)
  const acceptedOffersToComplete = myOffers.filter(o =>
    o.status === "ACCEPTED" && o.homeownerId === user.id
  );

  // Completed offers I can review
  const reviewedOfferIds = new Set(myReviews.map(r => r.offerId));
  const offersToReview = myOffers.filter(o =>
    o.status === "COMPLETED" && !reviewedOfferIds.has(o.id) && o.homeownerId === user.id
  );

  return `
Current State for ${user.fullName} (${user.role}):
- My Jobs Posted: ${myJobs.length} (Open: ${myJobs.filter(j => j.status === "OPEN").length}, In Progress: ${myJobs.filter(j => j.status === "IN_PROGRESS").length}, Completed: ${myJobs.filter(j => j.status === "COMPLETED").length})
- My Applications: ${myApplications.length}
- Available Jobs to Apply: ${availableJobs.length}
- Applications to My Jobs: ${applicationsToMyJobs.length} (Pending review)
- My Offers: ${myOffers.length} (Pending: ${myOffers.filter(o => o.status === "PENDING").length}, Accepted: ${myOffers.filter(o => o.status === "ACCEPTED").length})
...
- My Reviews Given: ${myReviews.filter(r => myOffers.some(o => o.id === r.offerId && o.homeownerId === user.id)).length}

Recent Activity:
${state.actionHistory.slice(-5).map(a => `- ${a.agentType}: ${a.action} (${a.success ? "success" : "failed"})`).join("\n") || "No recent activity"}
`.trim();
}
 ...
 */
export function getAvailableActions(state: TestAgentState, user: TestUser): AnyAction[] {
 ...
 */
function getAvailableHomeownerActionsForUser(state: TestAgentState, user: TestUser): HomeownerAction[] {
  ...
  const applicationsToMyJobs = state.applications.filter(a => myJobIds.has(a.jobId));
  ...
  const unreviewedOffers = completedOffers.filter(o => !reviewedOfferIds.has(o.id));
 ...
 */
function getAvailableTradespersonActionsForUser(state: TestAgentState, user: TestUser): TradespersonAction[] {
  const actions: TradespersonAction[] = ["browse_jobs", "view_offers", "view_reviews"];

  // Check for jobs I can apply to
  const myApplications = state.applications.filter(a => a.applicantId === user.id);
  const appliedJobIds = new Set(myApplications.map(a => a.jobId));
  const availableJobs = state.jobs.filter(j =>
    j.status === "OPEN" && !appliedJobIds.has(j.id) && j.createdBy !== user.id
  ...
  const pendingOffers = state.offers.filter(o =>
    o.tradespersonId === user.id && o.status === "PENDING"
  );
 ...
 */
function getAvailableBusinessActionsForUser(state: TestAgentState, user: TestUser): BusinessAction[] {
...
export async function decideNextAction(
  ...
  const prompt = `You are an autonomous agent acting as a ${user.role} in a job marketplace simulation.
...
Based on the current state and your goals, decide which action to take next.
If there's nothing productive to do, respond with "WAIT".

Think step by step:
1. What is the current state?
2. What progress have I made toward my goals?
...
For params, include relevant IDs if needed:
- apply_to_job: {"jobId": "uuid"} (optional, will pick first available if not specified)
- make_offer: {"jobId": "uuid", "tradespersonProfileId": "uuid", "budget": number}
- accept_offer/reject_offer: {"offerId": "uuid"}
- complete_job: {"offerId": "uuid"}
- leave_review: {"offerId": "uuid", "rating": 5, "feedback": "text"}

If no action needed, use: {"action": "WAIT", "reasoning": "explanation"}`;
  ...
  try {
    ...
    if (!availableActions.includes(decision.action)) {
      console.log(chalk.yellow(`  LLM suggested unavailable action: ${decision.action}, using heuristics`));
      return decideWithHeuristics(state, user, availableActions);
    }
    ...
    console.log(chalk.yellow(`  LLM decision failed, using heuristics: ${error}`));
    return decideWithHeuristics(state, user, availableActions);
  }
}
...
function decideWithHeuristics(
  ...
  const priorityOrder: Record<UserRole, AnyAction[]> = {
    homeowner: ["leave_review", "complete_job", "make_offer", "view_applications", "create_job"],
    tradesperson: ["accept_offer", "apply_to_job", "browse_jobs", "view_offers", "view_reviews"],
    business: ["leave_review", "complete_job", "make_offer", "view_applications", "create_job", "view_offers"],
    team_member: [],
    admin: [],
  };
...
function generateActionParams(
  ...
  const myJobs = state.jobs.filter(j => j.createdBy === user.id);
  const myJobIds = new Set(myJobs.map(j => j.id));
  const myOffers = state.offers.filter(o => o.homeownerId === user.id || o.tradespersonId === user.id);
  const myApplications = state.applications.filter(a => a.applicantId === user.id);

  switch (action) {
    case "apply_to_job": {
      const appliedJobIds = new Set(myApplications.map(a => a.jobId));
      const availableJob = state.jobs.find(j =>
        j.status === "OPEN" && !appliedJobIds.has(j.id) && j.createdBy !== user.id
      );
      if (availableJob) {
        return {
          apply_to_job: {
            ...
            estimatedCost: Math.floor(availableJob.budget * (0.85 + Math.random() * 0.2)),
            message: "I'm interested in this job and have the required experience.",
          },
        };
      }
      break;
    ...
    case "make_offer": {
      const applicationsToMyJobs = state.applications.filter(a => myJobIds.has(a.jobId));
      const pendingApplication = applicationsToMyJobs.find(a => a.status === "UR" || a.status === "S");
      if (pendingApplication) {
        const job = state.jobs.find(j => j.id === pendingApplication.jobId);
        // Find the tradesperson profile ID for this applicant
        const applicant = state.testUsers.find(u => u.id === pendingApplication.applicantId);
        if (job && applicant?.tradespersonProfileId) {
          return {
            make_offer: {
    ...
    case "accept_offer":
    case "reject_offer": {
      const pendingOffer = myOffers.find(o =>
        o.status === "PENDING" && o.tradespersonId === user.id
      );
      if (pendingOffer) {
    ...
    case "complete_job": {
      const acceptedOffer = myOffers.find(o =>
        o.status === "ACCEPTED" && o.homeownerId === user.id
      );
      if (acceptedOffer) {
    ...
    case "leave_review": {
      const reviewedOfferIds = new Set(state.reviews.map(r => r.offerId));
      const completedOffer = myOffers.find(o =>
        o.status === "COMPLETED" &&
...
export async function executeAction(
  ...
  try {
    ...
    switch (user.role) {
      case "homeowner":
        result = await homeownerAgent(state, decision.action as any, decision.params as any);
        break;
      case "tradesperson":
        result = await tradespersonAgent(state, decision.action as any, decision.params as any);
        break;
      case "business":
        result = await businessAgent(state, decision.action as any, decision.params as any);
        break;
      default:
 ...
 */
export function checkGoalsCompleted(state: TestAgentState, goals: SimulationGoals): boolean {
  return (
    state.jobs.length >= goals.minJobs &&
    state.applications.length >= goals.minApplications &&
    state.offers.filter(o => o.status === "ACCEPTED" || o.status === "COMPLETED").length >= goals.minAcceptedOffers &&
